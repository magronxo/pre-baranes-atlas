(()=>{
const svg=document.getElementById('g'),nodes=document.getElementById('nodes'),edges=document.getElementById('edges'),save=document.getElementById('save'),NS='http://www.w3.org/2000/svg',RUN='pre-baranes-atlas:runtime-boxes:v1';
...
function coding(){return [...svg.querySelectorAll('.runtime-box-owned')].find(g=>(g.textContent||'').includes('Coding Agents'))||null}
...
function persistRuntime(){const out={};for(const g of svg.querySelectorAll('.runtime-box-owned')){const p=T(g),r=g.querySelector('rect'),id=(g.textContent||'').includes('Coding Agents')?'coding':'ocws';out[id]={x:p.x,y:p.y,w:r?+r.getAttribute('width'):300,h:r?+r.getAttribute('height'):150}}try{localStorage.setItem(RUN,JSON.stringify(out))}catch(_){}}
if(save)save.addEventListener('click',()=>requestAnimationFrame(persistRuntime));
let scheduled=false;new MutationObserver(ms=>{if(!scheduled&&ms.some(m=>m.type==='attributes'||m.addedNodes.length||m.removedNodes.length)){scheduled=true;requestAnimationFrame(()=>{scheduled=false;draw()})}}).observe(svg,{subtree:true,childList:true,attributes:true,attributeFilter:['transform']});
draw();
})();