...
let scheduled=false;const observer=new MutationObserver(ms=>{if(!scheduled&&ms.some(m=>m.type==='attributes'||m.addedNodes.length||m.removedNodes.length)){scheduled=true;requestAnimationFrame(()=>{scheduled=false;draw()})}});
