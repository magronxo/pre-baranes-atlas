...
function pre(){const g=nodes.querySelector('.container-node[data-id="prebaranes"]'),p=T(g);return g?{x:p.x,y:p.y}:null}
function source(id){if(id==='architect')return nodes.querySelector('.routing-module');if(id==='research')return nodes.querySelector('.research-module');if(id==='writer')return nodes.querySelector('.writer-module');return nodes.querySelector(`[data-delivery-id="${id}"]`)}
...
function removeDuplicates(id,keep){let sel=id==='architect'?'.routing-module':id==='research'?'.research-module':id==='writer'?'.writer-module':`[data-delivery-id="${id}"]`;nodes.querySelectorAll(sel).forEach(g=>{if(g!==keep)g.remove()})}
...
function sync(){for(const id of IDS){const g=migrate(id);if(g)removeDuplicates(id,g)}redraw()}
