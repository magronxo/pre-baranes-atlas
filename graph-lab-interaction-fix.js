...
let drawScheduled=false,restoreScheduled=false;
const scheduleDraw=()=>{if(drawScheduled)return;drawScheduled=true;requestAnimationFrame(()=>{drawScheduled=false;draw()})};
const scheduleRestore=()=>{if(restoreScheduled)return;restoreScheduled=true;requestAnimationFrame(()=>{restoreScheduled=false;restoreWriter();draw()})};
new MutationObserver(ms=>{if(ms.some(m=>m.type==='childList'))scheduleRestore();else scheduleDraw()}).observe(nodes,{subtree:true,childList:true,attributes:true,attributeFilter:['transform']});
if(runtime)new MutationObserver(scheduleDraw).observe(runtime,{subtree:true,childList:true,attributes:true,attributeFilter:['transform']});
scheduleRestore();
})();