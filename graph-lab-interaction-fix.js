...
let scheduled=false;const sync=()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;draw()})};
new MutationObserver(sync).observe(nodes,{subtree:true,childList:true,attributes:true,attributeFilter:['transform']});
if(runtime)new MutationObserver(sync).observe(runtime,{subtree:true,childList:true,attributes:true,attributeFilter:['transform']});
sync();
})();